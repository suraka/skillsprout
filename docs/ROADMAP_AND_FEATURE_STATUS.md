...
| EDU-M2 / MATH-01…06 | PARTIAL | Draft Number Garden has fixed one-to-one counting, quantity comparison, add-one/take-one interactions and deterministic retry; no saved evidence or backend calls. | Qualified review, selected locale, audio/narration decision, manual accessibility/device/family checks and the remaining number outcomes are still required. |
...
5. **EDU-M2 mathematics (PARTIAL; draft only):** Number Garden provides count/compare/add-one/take-one interactions with fixed quantities and local-only progress. Runtime, typecheck, focused lint and production build pass locally; automated browser execution awaits CI because the local Chromium binary is unavailable. See EARLY_MATH_DRAFT.md. Three reviewed units, locale/narration, qualified math/accessibility/safety review and real device/family checks remain open.
...
- [x] Run the full runtime suite (26 cases), TypeScript, focused ESLint and production build with `/learning/number-garden` included.
- [ ] Run EDU-MB00…02 browser tests in GitHub Chromium CI; local run is blocked because the Playwright Chromium executable is absent.
- [ ] Qualified mathematics, language/locale, accessibility, safety/asset and family/device reviews; EDU-M2 remains partial and unpublished.

Continue by reading this checklist, MASTER_BLUEPRINT.md, FOUNDATIONAL_CURRICULUM.md, EARLY_MATH_DRAFT.md and TEST_PLAN_AND_RESULTS.md. Phase 1B and the literacy preview have passing automated checks; their qualified educator, assistive-technology, physical-device and family reviews remain open. The first Number Garden mathematics draft is implemented as a guest-only count/compare/add/remove activity but is not a reviewed three-unit EDU-M2 course. Do not publish curriculum before qualified review. Next work should run EDU-MB00…02 in Chromium CI, review any failures, then complete reviewer-led content and manual checks. Existing lesson evidence remains untouched; no migration was run.
