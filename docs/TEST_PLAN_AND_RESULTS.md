...
Version 20 adds quarter-turn direction, rectangle perimeter, illustrated ruler length from a nonzero mark, same-day elapsed whole hours, mass in identical model units, qualitative likelihood, equivalent-ratio scaling, and an add-three function rule. Local `pnpm test:runtime` passed all 59 runtime tests (38 Number Garden); `pnpm exec tsc --noEmit`, targeted ESLint, `pnpm build`, and `git diff --check` passed. The browser suite contains 38 cases covering retry and representation limits; local execution is blocked because Playwright Chromium is not installed. GitHub CI [run 35926350295](https://github.com/suraka/skillsprout/actions/runs/35926350295) passed all 59 runtime tests, all 38 Chromium browser tests, TypeScript and production build on commit `ca1ea8a5a322e3a66e597f234acc0be2ec10394a`. New v20 copy remains a draft pending recorded human review.
