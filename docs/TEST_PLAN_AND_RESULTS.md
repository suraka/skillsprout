...
Version 18 adds a Ghana cedi value prompt (GH¢2 = 200 pesewas; GH¢1 + 50 pesewas = GH¢1.50), a further unit-cube volume comparison, and a fuller MATH-06 path for building a chart from made-up counts, checking a claim, extending a repeating shape sequence, and selecting its rule. Currency calculations use integer pesewas; coin screens are text-only and do not solicit cash or purchases. Runtime tests cover supported denominations, whole-pesewa sums, bounded cube dimensions, exact chart rows, unique-maximum claim checks, and the alternating rule. Browser tests cover recovery and accessible controls for all three additions. A local browser run could not execute: all 35 cases stopped before test launch because this environment lacks the Playwright Chromium headless-shell executable. No browser assertion failed locally; GitHub CI is required to complete this gate. The user reports reviewing the source prompts for the MATH-05 work; the new MATH-06 chart/claim/rule wording needs review. The activity remains draft.
...
Version 19 adds quarter-turn direction, rectangle perimeter, illustrated ruler length from a nonzero mark, same-day elapsed whole hours, mass in identical model units, qualitative likelihood, equivalent-ratio scaling, and an add-three function rule. Local `pnpm test:runtime` passed all 59 runtime tests (38 Number Garden); `pnpm exec tsc --noEmit`, targeted ESLint, `pnpm build`, and `git diff --check` passed. The browser suite contains 38 cases covering retry and representation limits; local execution is blocked because Playwright Chromium is not installed. GitHub CI [run 35926350295](https://github.com/suraka/skillsprout/actions/runs/35926350295) passed all 59 runtime tests, all 38 Chromium browser tests, TypeScript and production build on commit `ca1ea8a5a322e3a66e597f234acc0be2ec10394a`. New v19 copy remains a draft pending recorded human review.

Currency references checked 2026-09-23: [Bank of Ghana Act, section 37](https://www.bog.gov.gh/wp-content/uploads/2019/09/banking-and-financial-laws-of-ghana-1998-2006.pdf) defines one cedi as 100 pesewas; [Bank of Ghana public notice on Ghana cedi coins](https://www.bog.gov.gh/wp-content/uploads/2026/07/Notice-No.23-BOG-SEC-GOV-2026-REJECTION-OF-GHANA-CEDI-COINS.pdf) identifies GH¢1, GH¢2 and pesewa denominations.
