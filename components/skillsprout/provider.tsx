...
export function AcademyProvider({children}:{children:React.ReactNode}){
 const [config,setConfig]=useState<Config|null>(null),[ready,setReady]=useState(false),[courses,setCourses]=useState<Course[]>(sampleCourses),[students,setStudents]=useState<Student[]>([]),[enrollments,setEnrollments]=useState<Enrollment[]>([]),[user,setUser]=useState<User|null>(null),[error,setError]=useState(''),[completed,setCompleted]=useState<Record<string,string[]>>({});
 const demo=ready&&config!==null&&!config.apiUrl;
 useEffect(()=>{fetch('/api/config').then(r=>{if(!r.ok)throw Error('Configuration unavailable');return r.json() as Promise<Config>;}).then(async(c:Config)=>{setConfig(c);if(c.apiUrl){setCourses([]);setCourses(await request<Course[]>(c,'/courses'));}else{setStudents([{id:'sample-learner',first_name:'Alex',age_band:'8-10'}]);}setReady(true);}).catch(()=>{setError('We could not connect. Please reload to try again.');setReady(true);});},[]);
