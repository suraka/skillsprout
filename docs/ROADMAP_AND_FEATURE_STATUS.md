...
4. **M1C / EDU-M0/M1 (PARTIAL; blocked for release):** English selected for the draft; added an original three-activity sound→letter→word preview, a clearly labeled link from Explore, fixed local checks, an acyclic outcome graph, publication-gated manifests, adult/offline guidance and transient grown-up recap. Local runtime/typecheck/targeted lint/build pass; browser CI is pending. See FOUNDATIONAL_CURRICULUM.md. A qualified literacy/pronunciation/accessibility/safety review, a reviewed locale and audio choice, and actual device/screen-reader/family checks remain required before publication. No curriculum outcome is marked approved.
...
- [ ] Run the new browser tests in GitHub CI and record that exact run before calling browser behavior verified.
...
Continue by reading this checklist, MASTER_BLUEPRINT.md, FOUNDATIONAL_CURRICULUM.md and TEST_PLAN_AND_RESULTS.md. Fetch current development branches/PRs; preserve user edits. Phase 1B code and automated verification are complete; its physical-device, screen-reader, qualified early-years and adult-assisted family reviews remain open. Phase 1C now has an English draft outline and a working guest preview. Its CI passed on commit `41c6acdb625b8fe54b6c439c95560f135b70a891`: all 17 Chromium browser tests, 21 runtime tests, TypeScript and production build. Literacy review, locale, media and manual release gates remain pending. Do not jump to generic subject pages. Draft LIT outcomes are not published or approved; no migration was run.
