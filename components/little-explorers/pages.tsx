...
export function RainbowHabitat() {
  ...
  return <Frame title="Rainbow Habitat"><main className="rh-page rh-game"><p className="rh-kicker">A GROWN-UP CAN HELP</p><h1>Rainbow Habitat</h1><p>Look at the garden friend. Choose a home with the same shape and pattern.</p>
    {finished ? <section className="rh-finish" aria-labelledby="rh-finished"><h2 id="rh-finished">All three friends found a home.</h2><p>No score is kept. Would you like to stop here?</p><p className="rh-offline"><strong>Try away from the screen:</strong> Find two safe household objects with a grown-up. What looks the same? What looks different?</p><div className="rh-actions"><button className="rh-button" onClick={restart}>Start again</button><button className="rh-button secondary" onClick={goHome}>Finish and go home</button></div></section> : <>
      {paused ? <section className="rh-pause" aria-live="polite"><h2>Paused</h2><p>The garden will wait. Take a break together.</p></section> : round && <section className={`rh-round${settings.motion&&bloom?' rh-bloom':''}`} aria-label={`Garden turn ${round.number} of 3`}>
      ...
      <div className="rh-actions"><button className="rh-button secondary" disabled={!ready} onClick={()=>{setPaused(!paused);if(paused)setFeedback('Let’s continue when you are ready.');}}>{paused?'Resume':'Pause'}</button><button className="rh-button secondary" disabled={!ready} onClick={goHome}>Home</button><Link className="rh-button secondary" href="/little-explorers/grownups">For grownups</Link></div>
    </>}
  </main></Frame>;
}
