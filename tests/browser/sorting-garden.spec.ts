...
test('SG-B03: keyboard path, pause and single step',async({page})=>{
 await page.goto(demo);await page.getByLabel('Sample card in block 2').focus();await page.keyboard.press('End');await page.keyboard.press('Tab');await expect(page.getByLabel('Sample card in block 2')).toHaveValue('green-round');
...
test('SG-B05: offline after load, mobile reflow and reduced motion',async({page,context})=>{
 await page.setViewportSize({width:360,height:800});await page.emulateMedia({reducedMotion:'reduce'});await page.goto(demo);await expect(page.getByLabel('Sample card in block 2')).toBeVisible();await context.setOffline(true);await page.getByRole('button',{name:'Run',exact:true}).click();await expect(page.getByText('This card reached the matching basket.',{exact:false})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);await page.getByRole('button',{name:'Finish for now'}).click();await expect(page.getByRole('heading',{name:'A good place to pause.'})).toBeVisible();
});
