...
| EDU-M2 / MATH-01…06 | PARTIAL | Number Garden's three MATH-01 lessons and v2 prompts are user-reported reviewed; MATH-02 compose/decompose prompts were reviewed and approved by the user. Version 9 includes user-reviewed MATH-03 and MATH-04 prompts through decimal/percent, plus a new draft fraction-number-line comparison with deterministic retry; no saved evidence or backend calls. | Review the new fraction-comparison prompt; complete remaining MATH-04 rational-number outcomes, then build MATH-05…06 as separate draft slices. Formal review records are not attached. |
...
5. **EDU-M2 mathematics (PARTIAL; draft only):** Number Garden's three MATH-01 lessons cover count, zero/order and comparison. Optional MATH-02 practice joins and splits groups and adds/takes one; the user approved its prompts. Version 6 added draft MATH-03 equal groups, arrays, and equal sharing. Version 7 added a small MATH-04 preview for tens/ones and fractions. The user reports reviewing MATH-03 and initial MATH-04 prompts. Version 8 added the ten-part decimal/percent preview, which the user reports reviewing. Version 9 adds a new draft fraction-number-line comparison prompt that needs review. Remaining MATH-04 outcomes and MATH-05…06 are still outstanding. Keep the whole activity in draft.
...
Continue by reading this checklist, MASTER_BLUEPRINT.md, FOUNDATIONAL_CURRICULUM.md, EARLY_MATH_DRAFT.md and TEST_PLAN_AND_RESULTS.md. Number Garden's original v2 content is reported reviewed by the product owner; new math content requires review. Other early-years and literacy slices still have their own review gates. Next math work is review of the new MATH-04 fraction-number-line prompt, then complete the remaining MATH-04 objectives and build MATH-05…06 as separate tested draft slices. Existing lesson evidence remains untouched; no migration was run.
