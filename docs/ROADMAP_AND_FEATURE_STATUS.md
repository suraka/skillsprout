...
| EDU-M2 / MATH-01…06 | PARTIAL | Draft Number Garden is grouped into three lessons (count; zero and order; compare), followed by optional add-one/take-one practice; deterministic retry, no saved evidence or backend calls. Product owner reports all reviews complete and is satisfied with the v2 prompts, choices, English locale, age/ability fit, accessibility and safety. | Reviewer findings/sign-off and device/family review notes are not recorded in this repository; the remaining number outcomes are still required. |
...
5. **EDU-M2 mathematics (PARTIAL; draft only):** Number Garden provides count/zero/order/compare/add-one/take-one interactions with fixed quantities and local-only progress. Runtime, typecheck, focused lint, production build and Chromium CI pass. See EARLY_MATH_DRAFT.md and TEST_PLAN_AND_RESULTS.md. Three reviewed units, locale/narration, qualified math/accessibility/safety review and real device/family checks remain open.
...
- [ ] Record reviewer findings/sign-off and real device/family evidence; EDU-M2 remains partial and unpublished.
...
Continue by reading this checklist, MASTER_BLUEPRINT.md, FOUNDATIONAL_CURRICULUM.md, EARLY_MATH_DRAFT.md and TEST_PLAN_AND_RESULTS.md. Phase 1B, literacy and the Number Garden draft have passing automated checks; qualified educator, assistive-technology, physical-device and family reviews remain open. Number Garden covers foundational number-sense interactions plus draft addition/subtraction practice, but is not a reviewed three-unit EDU-M2 course. Do not publish curriculum before qualified review. Next work is reviewer-led content and manual checks. Existing lesson evidence remains untouched; no migration was run.
