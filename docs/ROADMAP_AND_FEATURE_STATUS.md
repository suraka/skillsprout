...
| EDU-M2 / MATH-01…06 | PARTIAL | Number Garden prompts through version 17 are user-reported reviewed; version 16 adds a synthetic-data table and version 17 a repeating-shape pattern. The user also reports reviewing planned real-currency/further MATH-05 materials; those lessons remain unimplemented. No saved learner evidence or backend calls. | Implement the reported-reviewed real-currency/further MATH-05 lessons and remaining mass/time/3D geometry; complete MATH-06 separately. Formal review records are not attached. |
